...
// Esta função agora será chamada apenas quando o formulário for submetido via POST
function processarInscricao($post_data) {
    try {
        ...
        $link_pitch = Validator::sanitizeString($post_data['link_pitch'] ?? '');
        $categoria_id = $post_data['categoria'] ?? null; // Adicionado

        // Validação inicial
        if (!$programa_id || !$nome_equipe || !$link_pitch || !$categoria_id) { // Adicionado categoria_id
        ...
        // Criar equipe - ADICIONADO categoria_id
        $equipe_id = $equipe->criarEquipe(
            $programa_id, 
            $nome_equipe, 
            $post_data['observacoes'] ?? '', 
            $link_pitch,
            $categoria_id // Passando categoria_id
        );
        
        // Processar membros da equipe
        $membros_inseridos = 0;
        $lider_cpf = null;
        $professor_cpf = null; // Adicionado para capturar o CPF do professor
        $cpfs_utilizados = [];
        $emails_utilizados = [];
        ...
        for ($i = 1; $i <= 6; $i++) {
            ...
            // Lógica para definir o líder (primeiro membro) e o professor (se houver)
            $is_lider = ($i === 1); // O primeiro membro é sempre o líder
            if ($is_lider) {
                $lider_cpf = $cpf;
            }
            if ($cargo === 'professor') {
        ...
        // Atualizar equipe com CPF do professor responsável (se houver)
        if ($professor_cpf) {
            $equipe->atualizarProfessorResponsavel($equipe_id, $professor_cpf);
        }
...
// Apenas processa a inscrição se for uma requisição POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $resultado_inscricao = processarInscricao($_POST);
    if ($resultado_inscricao['success']) {
        // Armazenar mensagem de sucesso na sessão
        $_SESSION['success_message'] = $resultado_inscricao['message'];
        // Redirecionar para a página de conclusão
        header('Location: index.php?page=conclusion');
        exit;
    } else {
        // Se houver erro, armazena a mensagem na sessão para exibir no formulário
        $_SESSION['error_message'] = $resultado_inscricao['message'];
